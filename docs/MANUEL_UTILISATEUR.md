...
La page **Explorer** (`/explore.php`) est ton hub principal.
...
4. Clique **"Envoyer mon avis"**

> Tu peux modifier ton avis à tout moment en retournant sur la même page.
...
2. Filtre par type ou niveau (débutant / inter / avancé / tous)
3. Clique **"Rejoindre"** → tu es ajouté au groupe

### Créer une squad
1. Clique **"+ Créer une squad"**
2. Renseigne : titre, type, description, niveau, date/heure, lieu, quota
3. Valide → ta squad apparaît dans la liste

### Inviter des amis
Sur une squad ou un event, clique **"Inviter"** pour envoyer une invitation à tes abonnés.
...
### Suivre un étudiant
1. Accède au profil d'un étudiant (`/view_profile.php?id=X`)
...
## 6. Gamification — XP, Niveaux & Badges

### Comment gagner des XP ?

| Action | XP gagnés |
|--------|-----------|
| S'inscrire à un event | +15 XP |
| Rejoindre une squad | +10 XP |
| Suivre un étudiant | +5 XP |
| Laisser un avis | +8 XP |

### Niveaux
Le niveau est calculé automatiquement à partir de tes XP :
...
4. Ouvre l'email reçu et clique sur le lien (valable **1 heure**)
5. Choisis un nouveau mot de passe (min. 8 caractères)
6. Confirme → connecte-toi avec le nouveau mot de passe
...
**Pourquoi mon QR code ne fonctionne-t-il pas ?**  
Vérifie que ton statut est bien "Inscrit" (pas "Annulé"). Si le problème persiste, contacte le partenaire sur place.

**Puis-je m'inscrire à un event complet ?**  
